...
class AssessmentWizardV3 extends Page
{
    protected static ?string $navigationIcon = null; // Hidden from navigation
    protected static string $view = 'filament.pages.assessment-wizard-v3';
        ...
        return $form
            ->schema([
                Wizard::make([
                    ...
                    Wizard\Step::make('Informasi Dasar')
                        ...
                        ->schema([
                            Forms\Components\Section::make('Informasi Penilaian')
                                ->description('Data dasar untuk memulai penilaian')
                                ->schema([
                                    Forms\Components\Select::make('school_id')
                                        ...
                                        ->preload()
                                        ->live()
                                        ->afterStateUpdated(fn () => $this->validateSchoolSelection())
                    ...
                    // Step 2: Penilaian Skor
                    Wizard\Step::make('Penilaian Skor')
                        ->description('Berikan skor untuk setiap indikator')
                        ->icon('heroicon-m-star')
                        ->schema($this->getScoringStepSchema()),
    ...
    protected function getScoringStepSchema(): array
    {
        ...
        foreach ($categories as $category) {
            ...
            foreach ($category->indicators as $indicator) {
                $indicatorFields[] = Forms\Components\Group::make([
                    Forms\Components\Section::make($indicator->nama_indikator)
                        ->description($indicator->deskripsi_indikator)
                        ->headerActions([
                            FormAction::make('info')
                                ->label('Detail Kriteria')
                                ->icon('heroicon-m-information-circle')
                                ->color('info')
                                ->modalHeading('Kriteria Penilaian: ' . $indicator->nama_indikator)
                                ->modalContent(view('filament.components.assessment-criteria', [
                                    'indicator' => $indicator
                                ]))
        ...
        return [
            Forms\Components\Section::make('Ringkasan Penilaian')
                ...
                ->schema([
                    ...
                    Forms\Components\Placeholder::make('review_indicators')
                        ->label('Indikator Dinilai')
                        ->content(function (Forms\Get $get): string {
                            $scores = $get('scores') ?? [];
                            $validScores = array_filter($scores, fn($score) => !empty($score['skor']));
                            $totalIndicators = AssessmentIndicator::count();
                            return count($validScores) . ' dari ' . $totalIndicators . ' indikator';
                        }),

                    Forms\Components\Placeholder::make('review_files')
                        ->label('File Diupload')
                        ->content(function (Forms\Get $get): string {
                            $files = $get('files') ?? [];
                            $totalFiles = 0;
    ...
    protected function validateSchoolSelection(): void
    {
